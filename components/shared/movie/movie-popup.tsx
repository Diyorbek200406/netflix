  ...
  useEffect(() => {
    const getMovie = async () => {
      try {
        ...
        const findIndexOfTrailer = extractMovieDetails?.data?.videos?.results?.length ? extractMovieDetails?.data?.videos?.results?.findIndex((item: { type: string }) => item.type === "Trailer") : -1;
        const findIndexOfClip = extractMovieDetails?.data?.videos?.results?.length ? extractMovieDetails?.data?.videos?.results?.findIndex((item: { type: string }) => item.type === "Clip") : -1;
        const key = findIndexOfTrailer !== -1 ? extractMovieDetails?.data?.videos?.results[findIndexOfTrailer]?.key : findIndexOfClip !== -1 ? extractMovieDetails?.data?.videos?.results[findIndexOfClip]?.key : null;
        setKey(key);
      } catch (e) {
        console.log(e);
      } finally {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className={"flex flex-col space-y-4"}>
              <h1 className={"text-2xl md:text-4xl lg:text-4xl font-bold line-clamp-1"}>{movie?.title || movie?.name || movie?.original_name}</h1>
              <p className="text-shadow-md text-sm text-slate-500">{movie?.overview}</p>
            ...
            <div className={"bg-black p-4 rounded-md shadow-2xl"}>
              <h2 className="mt-2 mb-6 cursor-pointer text-sm font-semibold text-[#e5e5e5] transition-colors duration-200 hover:text-white md:text-2xl">More Like This</h2>
              <div className="grid grid-cols-4 gap-3 items-center scrollbar-hide md:p-2">{similarMovies && similarMovies.length && similarMovies.filter((item) => item.backdrop_path !== null && item.poster_path !== null).map((movie, index) => <MovieItem movie={movie} key={index} />)}</div>
            </div>
          </>
        )}
      </DialogContent>
