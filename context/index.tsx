...
const GlobalContext = ({ children }: ChildrenProps) => {
  ...
  return <Context.Provider value={{ account, setAccount, pageLoader, setPageLoader, open, setOpen, movie, setMovie }}>{children}</Context.Provider>;
};
export default GlobalContext;
export const useGlobalContext = () => {
  const context = useContext(Context);
  if (context === null) throw new Error("useGlobalContext must be used within a GlobalContext");
  return context;
};
